...
<div style="max-width: 1000px; margin: 0 auto; animation: fadeIn 0.6s ease;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 40px; border-bottom: 2px solid #e2e8f0; padding-bottom: 25px;">
        <div>
            <nav style="font-size: 0.8rem; color: #64748b; margin-bottom: 10px; font-weight: 600;">بوابة المدرس / سجل التسليمات / رصد الدرجات</nav>
            <h1 style="font-size: 2.2rem; font-weight: 800; color: #0f172a; letter-spacing: -1px;">مراجعة الحلول الأكاديمية 🖋️</h1>
            <p style="margin-top: 5px; color: #64748b;">رصد درجات الطالب: <strong style="color: var(--accent);">{{ $submission->student->name_ar }}</strong></p>
        </div>
        <div style="text-align: left; background: #fff; padding: 15px 30px; border-radius: 20px; border: 1px solid #e2e8f0; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
            ...
            <div style="font-size: 2.5rem; font-weight: 900; color: var(--accent); font-family: monospace;">{{ $submission->total_earned_grade ?? 0 }}</div>
        </div>
    </div>
    ...
    <form id="gradingForm">
        ...
        <div style="display: flex; flex-direction: column; gap: 30px;">
            ...
            <div class="glass-card" style="padding: 40px; border-right: 6px solid {{ $ans->question->type == 'mcq' ? '#10b981' : '#3b82f6' }}; border-radius: 25px; background: white; border: 1px solid #f1f5f9;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 20px;">
                    ...
                    <span class="chip" style="background: #f1f5f9; font-weight: 800; color: #1e293b;">الدرجة المستحقة للسؤال: {{ $ans->question->points }}</span>
                </div>
                ...
                <div style="background: #f8fafc; padding: 25px; border-radius: 20px; border: 1px solid #e2e8f0;">
                    ...
                    @else
                        ...
                        <div style="margin-bottom: 20px;">
                            ...
                            @if($ans->file_path)
                                <a href="{{ asset('storage/'.$ans->file_path) }}" target="_blank" class="btn" style="margin-top: 15px; background: white; border: 1px solid #cbd5e1; color: #0f172a; font-weight: 700; display: inline-flex; align-items: center; gap: 10px; padding: 10px 20px; border-radius: 12px;">
        ...
        <div style="margin-top: 60px; text-align: center; padding-bottom: 80px;">
            <button type="button" onclick="submitGradesToDB()" id="submitBtn" class="btn btn-primary" style="padding: 22px 120px; font-size: 1.4rem; border-radius: 30px; box-shadow: 0 15px 35px rgba(16, 185, 129, 0.25);">
...
<style>
    ...
    .f-input:focus { border-color: var(--accent); box-shadow: 0 0 0 4px rgba(16, 185, 129, 0.1); }
...
<script>
    function submitGradesToDB() {
        ...
        // إرسال الطلب عبر POST ليتوافق مع السيرفر ويمنع خطأ 405
        axios.post("{{ route('admin.submissions.saveGrade', $submission->id) }}", formData)
        .then(res => {
            if(res.data.success) {
                Swal.fire({
                    icon: 'success',
                    ...
                    // العودة لجدول التسليمات التفصيلي
                    window.location.href = "{{ route('admin.submissions.index') }}";
                });
            }
        })
        .catch(err => {
            console.error(err);
            Swal.fire('خطأ!', 'حدثت مشكلة أثناء الحفظ، يرجى المحاولة لاحقاً', 'error');
            btn.disabled = false;
